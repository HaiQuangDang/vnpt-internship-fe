...
const Service = ({ setAuth }) => {
    ...
    const getPlan = async () => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/dashboard/services/${serviceID}`,
                {
                    method: "GET",
                    headers: { token: localStorage.token },
                }
    ...
    const getServices = async () => {
        try {
            ...
            if (Array.isArray(parseRes)) {
                ...
                setServiceActive(parseRes.filter(parseRe => (parseRe.ma_dv == serviceID))[0]);
            }
        } catch (error) {
            console.error(error.message);
        }
    ...
    const sidebar = services.map(service => {
        ...
        return (
            <li key={service.ma_dv}
                style={serviceActive.ma_dv === service.ma_dv ? {
                    backgroundColor: '#fff'
                } : {}}>
                <Link className="link" to={serviceURL} onClick={clearMonthInput}>{service.ten_dv}</Link>
            </li>
        )
    })
    ...
    const deleteService = async () => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/admin/delete-serice/${serviceID}`,
                {
                    method: "DELETE",
                    headers: { token: localStorage.token },
                }
    ...
    const handleDeleteService = () => {
        if (window.confirm("Bạn có chắc chăn muốn xóa dịch vụ này?")) {
            deleteService();
        }
    }
        ...
        <div className="dashboard">
            ...
            <div className="dashboard-content">
                <div className="container-fluid">
                    <h1 className="title">{serviceActive.ten_dv}</h1>
                    <div className="row">
                        <div className="col-lg-3">
                            <div className="dashboard-sidebar">
                                <h3>Danh mục</h3>
                                <ul className="dashboard-sidebar_list">
                                    <li ><Link className="link" to="/">Tất cả</Link></li>
                                    {sidebar}
                                </ul>
                            </div>
                        </div>
                        <div className="col-lg-9">
                                ...
                                <table className="dashboard-table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style={{ width: "300px" }}>Kế hoạch</th>
                                            <th>Chỉ tiêu</th>
                                            <th>Thực hiện</th>
                                            <th>Doanh thu</th>
                                            <th>Trạng thái</th>
                            ...
                            {
                                plans.length === 0 && identity.phanquyen === 1 ? (
                                    <button type="button" onClick={() => handleDeleteService()}
                                        className="my-btn btn btn-danger mt-5">
                                        Xóa dịch vụ này
                                    </button>
                                ) : (<></>)
                            }
